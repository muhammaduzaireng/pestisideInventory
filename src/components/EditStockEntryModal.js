  ...
  const handleSave = async (stockEntryId) => {
    ...
    try {
      ...
      const errorMessage = err.response?.status === 404
        ? 'Stock entry update is not available. The server does not support this feature.'
        : 'Failed to update stock entry: ' + (err.response?.data?.error || err.message);
      setError(errorMessage);
      onSuccess(err); // Propagate error to parent
    }
  };
          ...
          <TableBody>
            {stockEntries.map((entry) => (
              <TableRow key={entry.id}>
                <TableCell>{entry.product_name || 'Unknown Product'}</TableCell>
