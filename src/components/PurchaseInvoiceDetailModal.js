...
const PurchaseInvoiceDetailModal = ({ invoiceId, onClose }) => {
    ...
    const fetchInvoiceDetails = async () => {
      ...
      try {
        ...
        const parsedInvoice = {
          ...
          record_id: invoiceResponse.data.id,
          ...
          items: (invoiceResponse.data.items || []).map((item, index) => {
            ...
            return {
              ...
              subtotal: parseFloat((parseInt(item.added_stock || 0) * parseFloat(item.purchase_price || 0)).toFixed(2)) || 0,
            };
          }),
        };
        ...
        if (err.code === 'ECONNRESET' && retryCount < maxRetries) {
          ...
          setError(
            err.response?.status === 404
              ...
              : `Failed to load invoice details: ${err.response?.data?.error || err.message}`
          );
        }
      } finally {
        setLoading(false);
  ...
  const generatePdfBill = () => {
    ...
    yPos += 7;
    doc.text(`Total Bill Amount: PKR ${formatCurrency(invoice.total_bill_amount)}`, 20, yPos);
    yPos += 7;
    doc.text(`Total Amount Paid: PKR ${formatCurrency(invoice.amount_paid)}`, 20, yPos);
    yPos += 7;
    doc.text(`Pending Credit: PKR ${formatCurrency(invoice.credit_amount)}`, 20, yPos);
    yPos += 7;
    ...
    if (paymentRecords.length > 0) {
      ...
      paymentRecords.forEach((payment) => {
        const rowData = [formatDate(payment.payment_date), `PKR ${formatCurrency(payment.amount_paid)}`];
    ...
    if (invoice.items && invoice.items.length > 0) {
      invoice.items.forEach((item) => {
        ...
        const rowData = [
          ...
          item.barcode || 'N/A',
          item.added_stock || 0,
          `PKR ${formatCurrency(item.purchase_price)}`,
          `PKR ${formatCurrency(item.sell_price)}`,
          formatDate(item.expiry_date),
          `PKR ${formatCurrency(itemSubtotal)}`,
        ];
        rowData.forEach((data, i) => {
      ...
      doc.setFontSize(10);
      doc.text('No items found for this invoice.', startX, yPos);
    }
  ...
  if (error) {
    return (
      ...
      >
        <Box sx={{ p: 2, bgcolor: 'background.paper', borderRadius: 2, color: 'error.main' }}>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Typography>
          <strong>Total Bill Amount:</strong> PKR {formatCurrency(invoice.total_bill_amount)}
        </Typography>
        <Typography>
          <strong>Payment Method:</strong> {invoice.payment_method.replace(/_/g, ' ').toUpperCase()}
        </Typography>
        <Typography>
          <strong>Total Amount Paid:</strong> PKR {formatCurrency(invoice.amount_paid)}
        </Typography>
        <Typography sx={{ color: invoice.credit_amount > 0 ? 'error.main' : 'success.main' }}>
          <strong>Pending Credit:</strong> PKR {formatCurrency(invoice.credit_amount)}
        </Typography>
                ...
                <TableBody>
                  {paymentRecords.map((payment) => (
                    <TableRow key={payment.id}>
                      ...
                      <TableCell align="right">PKR {formatCurrency(payment.amount_paid)}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
              ...
              {invoice.items && invoice.items.length > 0 ? (
                invoice.items.map((item) => (
                  <TableRow key={item.id}>
                    <TableCell>{item.product_name || 'N/A'} ({item.barcode || 'N/A'})</TableCell>
                    <TableCell>{item.barcode || 'N/A'}</TableCell>
                    <TableCell>{item.added_stock || 0}</TableCell>
                    <TableCell align="right">PKR {formatCurrency(item.purchase_price)}</TableCell>
                    <TableCell align="right">PKR {formatCurrency(item.sell_price)}</TableCell>
                    <TableCell>{formatDate(item.expiry_date)}</TableCell>
                    <TableCell align="right">PKR {formatCurrency(item.subtotal)}</TableCell>
                  </TableRow>
                ))
              ) : (
                <TableRow>
                  <TableCell colSpan={7} align="center">
                    <Typography color="text.secondary">No items found for this invoice.</Typography>
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
