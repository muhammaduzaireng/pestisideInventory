...
const CustomerRecords = () => {
  ...
  const fetchRecords = async () => {
    ...
    try {
      ...
      const sellRes = await axios.get('http://localhost:5001/api/sell-records', { params });
      setSellRecords(sellRes.data);

      // Fetch Credit Records for the selected customer
      const creditRes = await axios.get('http://localhost:5001/api/credit-records', { params });
      setCreditRecords(creditRes.data);

      // Fetch Credit Payments for the selected customer
      const paymentRes = await axios.get('http://localhost:5001/api/credit-payments', { params });
  ...
  const handleBillDetail = async (sellId) => {
    ...
    try {
      const res = await axios.get(`http://localhost:5001/api/sell/${sellId}`);
        ...
        <TableBody>
          {sellRecords.map(r => (
            <TableRow key={r.id}>
              ...
              <TableCell>${r.credit_amount}</TableCell>
              <TableCell>
