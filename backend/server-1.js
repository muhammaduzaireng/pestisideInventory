...
const pool = mysql.createPool({
  ...
  connectionLimit: 20, // Increased from 10 to handle load
  queueLimit: 0,
...
// DEPRECATED: Sell (Legacy endpoint, replaced by /api/sale-invoices)
...
// DEPRECATED: Add Sale (Legacy endpoint, replaced by /api/sale-invoices)
...
// Get Credit Payments (for CreditRecords)
...
// DEPRECATED: Credit Records (Legacy, replaced by /api/credit-records/sale-invoices)
...
// DEPRECATED: Add Credit Payment (Legacy for sales table)
...
app.get('/api/stock-purchases/:id', async (req, res) => {
  ...
  try {
    connection = await pool.getConnection();
    const [invoiceRows] = await queryWithRetry(
      `
      SELECT spi.*, v.name AS vendor_name
      FROM stock_purchase_invoices spi
    ...
    const [itemRows] = await queryWithRetry(
      `
...
app.get('/api/credit-records/customers', async (req, res) => {
  try {
...
app.get('/api/credit-records/sale-invoices', async (req, res) => {
  const { customer_id, start_date, end_date } = req.query;
